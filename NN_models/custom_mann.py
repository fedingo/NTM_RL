...
MANNControllerState = collections.namedtuple('MANNControllerState', ('controller_state', 'read_vector_list', 'w_list', 'M'))
...
class MANNCell(tf.contrib.rnn.RNNCell):
    ...
    def __call__(self, x, prev_state):
        ...
        with tf.variable_scope("o2p", reuse=tf.AUTO_REUSE):
        ...
        with tf.variable_scope("o2o", reuse=tf.AUTO_REUSE):
            MANN_output = []
            for read_vector in read_vector_list:
                output_elem = tf.contrib.layers.fully_connected(
                    tf.concat([controller_output, read_vector], axis=1), output_dim, activation_fn=None,
                    weights_initializer=self.o2o_initializer)
                output_elem = tf.clip_by_value(output_elem, -self.clip_value, self.clip_value)
                MANN_output.append(output_elem)

            MANN_output = tf.concat(MANN_output, axis = 1)

        self.step += 1
        self.memory_matrix = M

        return MANN_output, MANNControllerState(
    ...
    def output_size(self):
        return self.output_dim * self.read_head_num
